...
public class ChapterController {
    ...
    public ChapterSearchRes getChapters(@RequestParam(defaultValue = "0") int page,
                                        @RequestParam(defaultValue = "10") int pageSize,
                                        @RequestParam(defaultValue = "createdDate") String sort,
                                        @RequestBody(required = false) ChapterSearchReq req) {
        if (req == null) {
            req = new ChapterSearchReq();
        }

        return chapterService.getChapters(page, pageSize, sort, req);
    }
}
