...
//            "AND (:ratingFrom IS NULL OR (SELECT AVG(sc.rating) FROM UserCourse sc WHERE sc.course.id = c.id) >= :ratingFrom) " +
//            "AND (:ratingTo IS NULL OR (SELECT AVG(sc.rating) FROM UserCourse sc WHERE sc.course.id = c.id) <= :ratingTo)")
